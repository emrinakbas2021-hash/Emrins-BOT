...
async function playCommand(message, args) {
  ...
  try {
    ...
    if (!serverQueue) {
      const queueData = { textChannel: message.channel, voiceChannel, connection: null, player: null, songs: [videoInfo] };
      queues.set(message.guild.id, queueData);
      await loadingMsg.edit({ content: '', embeds: [nowPlayingEmbed(videoInfo)] });
      await startPlaying(message.guild.id, voiceChannel);
    } else {
      serverQueue.songs.push(videoInfo);
      await loadingMsg.edit({ content: '', embeds: [queuedEmbed(videoInfo, serverQueue.songs.length)] });
    }
  } catch (err) {
    console.error(err);
